              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant={'ghost'} size={'icon'} asChild>
                    <Link
                      prefetch={false}
                      href={`/edit-user/${user.user_id}?from=users`}
                    >
                      <PencilIcon size={16} />
                    </Link>
                  </Button>
    ...
    <div className='space-y-4 p-4 flex-1 overflow-y-auto'>
      {isLoading ? (
        ...
        <DataTableProvider table={table}>
          <DataTableHeader>
            <DataTableSearch placeholder={'Search user'} />
          </DataTableHeader>
          <DataTableWrapper>
            <DataTable />
            <DataTablePagination />
...
const UserLocations = ({ user }) => {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className='w-[640px] sm:max-w-none flex flex-col p-0'>
          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <TableRow key={headerGroup.id}>
              {headerGroup.headers.map((header) => (
                <TableHead key={header.id} className='py-3.5 px-6 text-xs'>
